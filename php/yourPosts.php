<?php 
    ...
    if ($stmt_recentPosts = $con->prepare('SELECT post_id, creator_name, date, title, post_body, private, num_comments FROM posts WHERE creator_id = ? ORDER BY date DESC LIMIT ' . $limit_offset . ',' . $limit_count)) {
